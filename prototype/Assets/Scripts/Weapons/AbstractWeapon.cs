...
public abstract class AbstractWeapon : MonoBehaviour
{
    ...
    IEnumerator updateGlowState()
    {
        while (true)
        {
            ...
            if (playerWeaponScript != null)
            {
                ...
                {
                    if (PlayerWeapons.getMainPlayerWeaponScript().getDPS() < this.getDPS()) // greenglow
                    {
                        changeSubpartsToMaterial(glowingModel, greenGlow);
                    }
                    else
                    {
                        changeSubpartsToMaterial(glowingModel, redGlow);
                    }
    ...
    IEnumerator glowLoop()
    {
        while (true)
        {
            if (glowingModel)
            {
                ...
                glowingModel.transform.localScale = GlowbaseScale * (Mathf.Sin(Time.time * pulseSpeed) * pulseSize + pulseSize+1f);
            }
            yield return null;
        }
    }
    ...
    private void changeSubpartsToMaterial(GameObject obj, Material material)
    {
        Renderer[] parts = glowingModel.GetComponentsInChildren<Renderer>();
