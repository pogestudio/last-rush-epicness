...
public class EffectFactory : MonoBehaviour
{
    ...
    public LineRenderer sniperShot;
    ...
    public void deliverBloodSpurtEffect(Vector3 origin, Vector3 direction)
    {
        networkView.RPC("deliverBloodSpurtEffectRPC", RPCMode.All, origin, direction);
    }

    [RPC]
    private void deliverBloodSpurtEffectRPC(Vector3 origin, Vector3 direction)
    {
    ...
    private void createLightningBetweenRPC(Vector3 originPos, Vector3 destPos, float lightningThickness)
    {
        ...
        while (Vector3.Distance(destPos, lastPoint) > .5)
    ...
    {
        networkView.RPC("createSniperShotRPC", RPCMode.All, start, end);
    }

    [RPC]
    private void createSniperShotRPC(Vector3 start, Vector3 end)
    {
        LineRenderer tmp = GameObject.Instantiate(sniperShot) as LineRenderer;
        tmp.SetVertexCount(2);
        tmp.SetWidth(0.1f, 0.1f);
        tmp.SetColors(Color.white, Color.white);
        tmp.SetPosition(0, start);
        tmp.SetPosition(1, end);
